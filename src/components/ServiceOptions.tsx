...
const ServiceOptions = ({ userType, serviceType, setServiceType }: ServiceOptionsProps) => {
		...
		return (
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
				...
				>
					...
					<h3 className="text-lg font-semibold mb-2 text-gray-900">방문 요양을 원해요</h3>
					<p className="text-sm opacity-80 text-gray-700">집으로 방문하는 돌봄 서비스</p>
				</button>
				<button
				...
				>
					<div className="text-4xl mb-4 text-gray-900">👪</div>
					<h3 className="text-lg font-semibold mb-2 text-gray-900">가족 요양을 원해요</h3>
					<p className="text-sm opacity-80 text-gray-700">가족이 돌봄을 제공하는 서비스</p>
				</button>
			</div>
		);
	}
	...
	return (
		<div className="space-y-6 text-center">
			<h3 className="text-2xl font-semibold text-gray-900">{detail?.title}</h3>
			<p className="text-gray-700">{detail?.desc}</p>
			<p className="font-medium text-gray-700">{detail?.price}</p>
